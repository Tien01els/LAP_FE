...
const StudentCard = ({ student, setStudentInfo, onGetStudentOfClass }) => {
  ...
  const handleRemoveStudent = async (id, accountId) => {
    try {
      await axiosJWT.delete(API_URL + `student/${id}/class`)
      await axiosJWT.delete(API_URL + `notification-room/receiver/${accountId}`)
      onGetStudentOfClass()
    } catch (err) {
      console.log(err)
    }
    ...
    <div className="w-full h-[140px] flex flex-row gap-4 bg-white rounded-[16px] items-center shadow-md hover:shadow-lg transition-all select-none px-3 py-3">
      ...
      <div className="flex flex-col justify-evenly h-full">
        <div className="flex flex-row items-center">
          <span
            className="font-medium w-[210px] truncate cursor-pointer"
              ...
              {openMoreOption && (
                <div className="absolute translate-y-8 -translate-x-5 border-t-2 text-sm border-primary bg-[#ffffff] flex flex-col divide-y shadow-lg rounded-b">
                  <div
                    ...
                    onClick={() =>
                      handleRemoveStudent(student?.id, student?.accountId)
                    }
                  >
                    <span>Remove</span>
                  </div>
        ...
        <p className="text-xs w-[240px] h-[48px] whitespace-normal break-words line-clamp-3">
        ...
        <div className="flex flex-row justify-between items-center mr-5 text-xs">
          <span>
            Average Score :{' '}
