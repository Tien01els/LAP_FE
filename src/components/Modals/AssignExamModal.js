...
const AssignExamModal = ({
  ...
  useEffect(() => {
    if (assignStudents?.length === students?.length) {
      return setAssignAll(true)
    }
    return setAssignAll(false)
  ...
  const assignForStudents = async () => {
    try {
      await axiosJWT.put(
        API_URL +
          `student-assignment/assignment/${currentAssignment.assignment?.id}/assign-list-student`,
        {
          listStudentId: assignStudents,
          dateDue: currentAssignment.dateDue,
        },
      )
      getAssignmentOfClass(currentAssignment.assignment?.id)
      setIsOpen(!isOpen)
  ...
  useEffect(() => {
    if (studentsOfAssignment && students) {
      ...
      for (let i = 0; i < studentsOfAssignment.length; ++i) {
        if (
          students.find(
            (student) => student.id === studentsOfAssignment[i].studentId,
          )
        )
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4 w-[500px]">
        ...
        <div className="flex flex-row-reverse gap-2 mx-3">
          <label
            htmlFor="assign-all"
            ...
            .map((val, i) => {
              return (
                ...
                >
                  ...
                  <span className="w-[70%] truncate inline">
                    {val.fullName}
                  </span>
                  <span
                    className={`${
