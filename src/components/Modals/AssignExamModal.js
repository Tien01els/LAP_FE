          ...
          {students.map((val, i) => {
            return (
              <div
                ...
                className="w-full flex flex-row justify-between bg-[#f4f7f9] gap-5 h-fit px-4 py-2 rounded-lg cursor-pointer"
              >
                <label
                  htmlFor={`student-${i}`}
                  className="flex items-center w-[50%] gap-4 cursor-pointer"
                >
                  <input
                    id={`student-${i}`}
                    type="checkbox"
                  ...
                  ></input>
                  <span className="w-[70%] truncate">{val.fullName}</span>
                </label>
