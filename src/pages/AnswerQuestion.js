...
const AnswerQuestion = ({ isTeacher }) => {
  ...
  const checkAnswered = (questionOfAssignment) => {
    ...
    if (
      ...
      if (typeOfquestion === 'trueFalse')
        for (let i = 0; i < resultOfUser.length; i++)
          if (resultOfUser[i].isTrue) return true
      if (typeOfquestion === 'input' && resultOfUser[0].answer.length > 0)
        return true
    ...
    try {
      if (isTeacher)
        !isSubmit &&
          (await axiosJWT.put(
            API_URL +
              `teacher-assignment/teacher/assignment/${assignmentId}/submit`,
          ))
      else
        !isSubmit &&
          (await axiosJWT.put(
            API_URL +
              `student-assignment/student/assignment/${assignmentId}/submit`,
          ))
  ...
  useEffect(() => {
    const countDownTime = async () => {
      ...
      if (isTeacher)
        res = await axiosJWT.get(
          API_URL +
            `teacher-assignment/teacher/assignment/${assignmentId}/do-time`,
        )
      else
        res = await axiosJWT.get(
          API_URL +
            `student-assignment/student/assignment/${assignmentId}/do-time`,
        )
      if (res) {
    ...
    return () =>
      clearTimer.then((clearTimerId) => clearTimerId && clearTimerId())
  ...
  useEffect(() => {
    if (currentQuestion) {
      ...
      setAnswers(
        currentQuestion.answerOfRespondent?.answer
          ? answerOfRespondent
          : newAnswer,
      )
    }
  ...
  const renderAnswer = (questionTypeId) => {
    switch (questionTypeId) {
      case 1:
        return (
          answers && (
            <MultiChoiceAnswer answers={answers} setAnswers={setAnswers} />
          )
        )
      ...
      case 3:
        return (
          answers && <InputAnswer answers={answers} setAnswers={setAnswers} />
        )
      // <textarea
      ...
      case 4:
        return (
          answers && (
            <MultiSelectAnswers answers={answers} setAnswers={setAnswers} />
          )
        )
      default:
        return <div>404</div>
      ...
      <div className="flex flex-row gap-5 h-fit w-full">
        ...
        <div className="flex flex-col w-[65%] gap-5">
          <div className="w-full bg-white shadow rounded-lg px-12 pt-7 pb-5 flex flex-col gap-5 text-justify">
            ...
            {/* answer */}
            <div className="">
              {renderAnswer(currentQuestion?.questionTypeId)}
            </div>
            {/* Next */}
