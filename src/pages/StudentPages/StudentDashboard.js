...
const StudentDashboard = ({ isParent }) => {
    ...
    try {
      if (isParent && studentId) {
        res = await axiosJWT.get(
          API_URL + `parent/student/${studentId}/deadline`,
        )
      } else {
        res = await axiosJWT.get(
  ...
  const getPercentSkill = async (studentId) => {
    try {
      ...
      if (isParent && studentId) {
        res = await axiosJWT.get(
          API_URL + `parent/student/${studentId}/percent-skill`,
        )
      } else {
  ...
  const getClassOfStudent = async (studentId) => {
    ...
    try {
      if (isParent) {
        res = await axiosJWT.get(API_URL + `parent/student/${studentId}/class`)
      } else {
  ...
  const getAchievementOfStudent = async (studentId) => {
    try {
      ...
      if (isParent) {
        res = await axiosJWT.get(
          API_URL + `parent/student/${studentId}/achievement`,
        )
      } else {
            ...
            <div className=" px-4 py-3 flex flex-col w-[60%] bg-white shadow-lg rounded-lg">
              <div className="flex flex-row items-center px-2 justify-between">
                ...
                <span
                  onClick={() =>
                    navigate(
                      `/class/${decodedToken?.classId || studentInfo?.classId}`,
                    )
                  }
                    ...
                    >
                      <div className="flex flex-col items-center justify-center text-primary">
                        <span className="font-semibold text-4xl">
                          {Math.round(achievement?.avgScoreOfStudent) || 0}
                        </span>
